...
SCREENSHOT_DIR = r"D:\project screenshot"
...
try:
    ...
    print("    Falling back to media keys.")
...
def change_volume(direction):
    if not VOLUME_AVAILABLE:
        ...
        pyautogui.press(key, presses=VOLUME_STEP_PERCENT, interval=0.02)
        print(f"[Volume] {'Up' if direction > 0 else 'Down'} by {VOLUME_STEP_PERCENT}")
        return
...
def open_ir_camera():
    ...
    # DSHOW first: many Windows IR devices open more reliably with it.
...
def main():
    ...
    while True:
        ...
        gesture = detect_gesture(finger_count, finger_state)
